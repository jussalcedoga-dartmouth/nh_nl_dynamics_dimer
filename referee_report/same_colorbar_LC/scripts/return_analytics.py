...
## I might as well just pass the vmin and max as arguments, tho.

def create_amplitude_plot(ax, vmin, vmax):
    ...
    print('Analytical Amplitude lims', np.nanmin(power_dBm), np.nanmax(power_dBm))

    # Plotting
    cf = ax.contourf(gain_grid, phase_grid, power_dBm, levels=200, cmap='inferno', vmin=vmin, vmax=vmax)
    # cbar_amp = plt.colorbar(cf, ax=ax, label='Power [dBm]')
...
def create_frequency_plot(ax, vmin, vmax):
    ...
    print('Analytical Frequency lims', np.nanmin(masked_frequency_grid),   np.nanmax(masked_frequency_grid))

    # Plotting the masked frequency grid
    cf_freq = ax.contourf(gain_grid, phase_grid, masked_frequency_grid, levels=200, cmap='inferno', vmin=vmin, vmax=vmax)
    # cbar_freq = plt.colorbar(cf_freq, label=r'$\delta \omega_{\rm{{LC}}}/2\pi$ [MHz]')
