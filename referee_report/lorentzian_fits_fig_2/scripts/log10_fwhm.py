...
UPDATE 2025-06-23
-----------------
• Replace natural-log linewidth with log10(FWHM) everywhere:
  – new columns `log10_fwhm`, `dlog10_fwhm` in the summary CSV  
  – twin-axis plot now shows log10(FWHM) with proper error bars  
• Axis/legend labels changed accordingly.
"""
...
def load_csv(path, is_exp, phase):
    ...
    df = df.dropna()
    if df.f.max() > 1e7:     # Hz → GHz
        df.f /= 1e9
    wl, wh = WIN_H if phase == "hermitian" else WIN_NH
    df = df[(df.f >= wl) & (df.f <= wh)]

    s21d  = df.p.values + (20 if is_exp else 0)   # exp traces need +20 dB offset
    s21l  = db2lin(s21d)
    f     = df.f.values
...
def half_width(f, y_db, idx):
    w = peak_widths(y_db, [idx], 3/DB)[0][0]   # 3 dB half-height
...
def add_row(table, g, side, A, eA, γ, eγ, C, ph, ds):
    table.append(dict(
        ...
        log10_fwhm=np.log10(γ*1e3),               # MHz
        dlog10_fwhm=log10err(γ*1e3, eγ*1e3),
    ))
...
if not REC:
    print("⚠️  No traces found – check paths & filenames.")
    sys.exit(0)
...
# Hermitian panel – peak amplitude
...
# Non-Hermitian panel – amplitude (red) & log10-FWHM (blue)
for ds in ["exp", "theory"]:
    sub = df[
...
for ds in ["exp", "theory"]:
    ...
    ax2b.errorbar(
        ...
        capsize=3, label=f"log10 FWHM ({ds})"
    )

ax2b.set_ylabel(r"$\log_{10}$ FWHM [MHz]", color="dodgerblue")
