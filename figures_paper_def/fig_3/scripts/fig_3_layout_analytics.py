...
# Setup figure and grid specification
fig = plt.figure(figsize=(17, 17))
gs = GridSpec(3, 2, figure=fig)  # Now using 3 rows instead of 2

# Define subplot positions in the grid
# Column 1
ax_amp_exp = fig.add_subplot(gs[0, 0])  # Top plot in the first column
ax_amp_num = fig.add_subplot(gs[1, 0])  # Bottom plot in the first column
ax_amp_ana = fig.add_subplot(gs[2, 0])  # Middle plot in the first column (analytic amplitude)

# Column 2
ax_freq_exp = fig.add_subplot(gs[0, 1])  # Top plot in the second column
ax_freq_num = fig.add_subplot(gs[1, 1])  # Bottom plot in the second column
ax_freq_ana = fig.add_subplot(gs[2, 1])  # Middle plot in the second column (analytic frequency)

# Adjust the spacing between plots
gs.update(wspace=0.2, hspace=0.25)  # Adjust the width and height spacing

# Amplitude of limit cycle (experimental)
...
# Analytic amplitude plot
create_amplitude_plot(ax_amp_ana)

# Frequency of limit cycle (experimental)
...
cbar_freq_exp = plt.colorbar(contour_freq_exp, label=r'$|\delta \omega_{\rm{{LC}}}^{\rm{{exp}}}|$ [MHz]', ax=ax_freq_exp)
# cbar_freq_exp = plt.colorbar(contour_freq_exp, label=r'$\Delta \omega_{\rm{{LC}}}^{\rm{{exp}}}$ [MHz]', ax=ax_freq_exp)

format_colorbar(cbar_freq_exp)

# Numerical frequency plot
...
cbar_freq_num = plt.colorbar(contour_freq_num, ax=ax_freq_num, label=r'$|\delta \omega_{\rm{{LC}}}^{\rm{{num}}}|$ [MHz]')
format_colorbar(cbar_freq_num)

# Analytic frequency plot
create_frequency_plot(ax_freq_ana)

for ax in [ax_amp_exp, ax_amp_ana, ax_amp_num, ax_freq_exp, ax_freq_ana, ax_freq_num]:
...
labels = ['experiment', 'numerics', 'theory']

# Apply text labels to the top left of each subplot in the left column
for ax, label in zip([ax_amp_exp, ax_amp_num, ax_amp_ana], labels):
...
ax_amp_ana.text(-0.15, 1.2, r'$\textbf{c}$', transform=ax_amp_ana.transAxes, fontsize=40, fontweight='bold', va='top', ha='right')

ax_freq_exp.text(-0.15, 1.2, r'$\textbf{d}$', transform=ax_freq_exp.transAxes, fontsize=40, fontweight='bold', va='top', ha='right')
ax_freq_num.text(-0.15, 1.2, r'$\textbf{e}$', transform=ax_freq_num.transAxes, fontsize=40, fontweight='bold', va='top', ha='right')
ax_freq_ana.text(-0.15, 1.2, r'$\textbf{f}$', transform=ax_freq_ana.transAxes, fontsize=40, fontweight='bold', va='top', ha='right')
