...
data_dir = 'single_traces_nonhermitian/data'
...
for file in files:
    ...
    if frequency is None:
        frequency = df['Frequency'].values/1e9  # Assume all files have the same frequency range
...
c = ax.imshow(power_db_matrix, aspect='auto', cmap='inferno', 
              extent=[frequency[0], frequency[-1], gains[-1], gains[0]], vmin=-45)

ax.set_xlabel('Frequency [GHz]')
ax.set_ylabel(r'$\Delta G$')
fig.colorbar(c, ax=ax, label=r'$S_{21}$ [dB]')
plt.tight_layout()
plt.savefig('transmission_non_hermitian.png')
plt.close()
...
plt.plot(gains, max_power_dB, marker='o', linestyle='-', color='blue')
plt.xlabel(r'$\Delta G$')
plt.ylabel(r'$S_{21}^{\rm{max}}$ [dB]')
plt.tight_layout()
plt.savefig('max_transmission_vs_gain_nonhermitian.png')
plt.close()
