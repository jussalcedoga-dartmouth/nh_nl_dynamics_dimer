...
## Equations of motion as written in the notes
...
h_bar = 1.054571817e-34
epsilon_dBm = -30  # dBm
## select either phase = 0 or phase = np.pi
# phase = np.pi
phase = 0.0
...
def simulate_frequency(args):
    ...
    # Time span is set so that we time evolve for 10000 times the characteristic dissipation rate kappa_c
    ### We need to time evolve for a long time to get good resolution in the FFT
    ...
    #### Time and photon numbers
    t = np.linspace(t_span[0], t_span[1], 100000)
    y = sol.sol(t)

    # Extract the different components of the signal
    alpha2_real = y[2]
    alpha2_imag = y[3]
    alpha2_complex = alpha2_real + 1j * alpha2_imag

    ### This is a very clean implementation!!!! Ditch the first 20%
    ...
    ### Photon numbers from the DC component directly
    ...
    ### Convert to actual power that we measure in the lab
...
def run_simulation_for_gain(net_gain):
    ...
    frequencies = np.linspace(5.975e9, 6.085e9, 500)
    results = []
    
    # Folder setup
    gain_folder = f'single_traces_Vincent_hermitian/data/'
    os.makedirs(gain_folder, exist_ok=True)
    csv_path = os.path.join(gain_folder, f'results_gain_{net_gain:.2f}.csv')
    
    # Folder setup
    gain_folder_images = f'single_traces_Vincent_hermitian/images/'
