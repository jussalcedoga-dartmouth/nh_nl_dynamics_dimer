...
net_gains = np.linspace(4, 8.4, 10)
phases = np.linspace(0, 2*np.pi, 20)
times_kappa_c = 500
amplitudes_LC = []

for net_gain in tqdm(net_gains):

    plot_folder = f'better_resolution/undriven_time_domain_net_gain_{net_gain:2f}'
    ...
    for phase in tqdm(phases):
        ...
        t_span = (0, times_kappa_c*(1/kappa_c))
        ...
        norm = np.sqrt(alpha2_real**2 + alpha2_imag**2)**2

        ## select just the last datapoint as the 'steady-state'
        final_norm_value = norm[-1]

        amplitudes_LC.append([net_gain, phase, final_norm_value])

        fig, axs = plt.subplots(1, 2, figsize=(12, 6))  # Adjust subplot to include FFT
        ...
        plt.tight_layout()
        plt.savefig(f'{plot_folder}/combined_plot_GHz_phase_{phase:2f}.png')
        plt.close()

df = pd.DataFrame(amplitudes_LC, columns=['net_gain', 'phase', 'amplitude_lc'])
df.to_csv('better_resolution/results.csv', index=False)
