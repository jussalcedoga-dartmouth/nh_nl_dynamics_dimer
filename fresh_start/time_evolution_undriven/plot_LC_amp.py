...
# Load results
df = pd.read_csv('group_meeting/results.csv')

# Prepare grid for plotting
phase_values = df['phase'].unique()
gain_values = df['net_gain'].unique()
phase_values.sort()
gain_values.sort()
...
for index, row in df.iterrows():
    phase_index = np.where(phase_values == row['phase'])[0][0]
    gain_index = np.where(gain_values == row['net_gain'])[0][0]

    ## convert photon numbers to power in dBm
    photon_numbers = row['amplitude_lc']
...
contourf_plot = plt.contourf(gain_values, phase_values, final_norm_matrix, levels=200, cmap='inferno', vmin=vmin)
...
cbar = clippedcolorbar(contourf_plot, label='Amplitude [dBm]')
...
plt.tight_layout()
plt.savefig('group_meeting/numerical_amplitude_limit_cycle.png')
plt.close()
