...
# Transmission amp: Fitted from the fits shown in the supplement (Fig. 5 b) amp 1.
gain_1 = G_0
a_1, b_1, c_1, flat_line_value_1, x0_1 =  9.177, 0.00839, 0.0930, 8.2716, 0.000798

# Transmission amp: Fitted from the fits shown in the supplement (Fig. 5 b) amp 2.
gain_2 = G_0
a_2, b_2, c_2, flat_line_value_2, x0_2 =  9.177, 0.00839, 0.0930, 8.2716, 0.000798

# Reflections off amp: Fitted from the fits shown in the supplement (Fig. 5 b) amp.
...
def func(alpha, omega_d, phase, attenuation, epsilon_dBm = -10):
    ...
    ### The crossover is determined as the gain value at which the 'non-linearity kicks in' at each phase.
    ### Movable offset, which allows us to 'discard' the explicit 'phi dependence' on the on-diagonal terms.
    ### These values are chosen based on experimental observations.
    ...
    eta_A = 10**(-attenuation/20) ## Atenuation eta
    eta_I = 10**(-insertion_loss/20) ## Insertion loss eta

    nu_G12 = piece_wise_amp_function(N1_watts, a_1, b_1, c_1, flat_line_value_1, x0_1) * eta_A * eta_I ## dimensionless gain term
    nu_G21 = piece_wise_amp_function(N2_watts, a_2, b_2, c_2, flat_line_value_2, x0_2) * eta_A * eta_I ## dimensionless gain term

    nu_r1 = piece_wise_amp_reflection(N1_watts, a_r, b_r, c_r, flat_line_r, x0_r)* eta_A * eta_I ## reflection coefficient
    nu_r2 = piece_wise_amp_reflection(N2_watts, a_r, b_r, c_r, flat_line_r, x0_r)* eta_A * eta_I ## reflection coefficient

    ### Reflections off the phase shifter. Fitted from the actual data at the two different phases
    ### Fig. 5.c - Supplement
    if phase == 0:
    ...
    nu_ps = model_function_reflection_ps(N1_watts, a_ps, b_ps, c_ps) ### reflection off the phase shifter
    ...
    print('kappa_1', kappa_diag_1/1e6)
    print('kappa_2', kappa_diag_2/1e6)
    ...
    d_alpha2 = -(kappa_diag_2 + 1j*(omega2 - omega_d))*alpha2_c \
                + 1j * J_12 * np.exp(-1j*phase) * alpha1_c + epsilon
