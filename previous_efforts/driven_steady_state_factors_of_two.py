...
folder_name = 'SS_results_factors_of_two2'
...
t_hop_1_1 = 16e6#.71e6## sigmax cavity 1

t_hop_1_2 = 16e6#.89e6## port 1 cavity 2

t_hop_2_1 = 16e6#.77e6## sigmaz cavity 1

t_hop_2_2 = 16e6#.94e6## sigmax cavity 2

# Drive 2 read 1
readout_kappa = 3.06e6# port 1 cavity 1
drive_kappa = 3.87e6# port sigmaz cavity 2

kappa_0_1 = 7.04e6 - 1.5e6
kappa_0_2 = 8.73e6 - 1.5e6
...
gain_1_offset = gain_1 - att1
gain_2_offset = gain_2 - att2

average_atten = np.mean([att1, att2])
gain_offset = np.mean([gain_1_offset, gain_2_offset])

print(f'gain_offset: {gain_offset}, average_atten: {average_atten},att1: {att1}, att2: {att2}')
...
def func(alpha, omega_d, phase, attenuation, epsilon_dBm = -10):
    ...
    G1 = piece_wise_amp_function(N1_watts, a_1, b_1, c_1, flat_line_value_1, x0_1) * 10 ** (-(attenuation + average_atten)/20)
    G2 = piece_wise_amp_function(N2_watts, a_2, b_2, c_2, flat_line_value_2, x0_2) * 10 ** (-(attenuation + average_atten)/20)
    ...
    d_alpha2 = -(((kappa_cavity2)/2 - (G1)*np.sqrt(t_hop_1_2 * t_hop_2_2)*np.exp(-1j * phase)/2) + 1j * (omega2 - omega_d)) * alpha2_c + 1j * G1 * np.sqrt(t_hop_1_2 * t_hop_2_2) * alpha1_c * np.exp(-1j * phase) + epsilon
...
def create_plots_for_phase(frequencies, attenuations, phase, epsilon_dBm=-10, gain_offset = 0):
    alpha2_solutions = np.empty((len(frequencies), len(attenuations)))

    print('This is the gain offset', gain_offset)

    gains = gain_offset - attenuations #+ 0.59

    print()
    print('Actual Gains', gains)
    print()
    ...
    if phase == 0:
        ...
        plt.plot(gains, max_values_below_center, marker='o', linestyle='-', color='b', label='left peak')
        plt.plot(gains, max_values_above_center, marker='o', linestyle='-', color='crimson', label='right peak')
        plt.xlabel('Net Gain [dB]', fontsize=20)
        plt.ylabel(r'$S_{21}^{\rm{max}}$(dB)', fontsize=20)
        plt.axvline(4.1, ls='--', lw=2.0, color='black')
        plt.axhline(0, ls='--', lw=2.0, color='black')
        plt.legend(fontsize=15)
        plt.gca().invert_xaxis()
        plt.gca().tick_params(axis='both', which='major', labelsize=20)
    else:
        plt.figure(figsize=(7, 4))
        plt.plot(gains, max_values, marker='o', linestyle='-', color='crimson',)
        plt.xlabel('Net Gain [dB]', fontsize=20)
        plt.ylabel(r'$S_{21}^{\rm{max}}$(dB)', fontsize=20)
        plt.axhline(0, ls='--', lw=2.0, color='black')
        plt.axvline(4.1, ls='--', lw=2.0, color='black')
    ...
    if phase == 0:
        plt.imshow(alpha2_solutions.T, extent=[frequencies[0]/1e9, frequencies[-1]/1e9,
                                            gains[-1], gains[0]],
                aspect='auto', cmap='inferno', vmax=-1)#, vmax=vmaxs[phase])
    else:
        plt.imshow(alpha2_solutions.T, extent=[frequencies[0]/1e9, frequencies[-1]/1e9,
                                            gains[-1], gains[0]],
...
alpha = [2000, 0, 2000, 0]
phases = [np.pi]
attenuations = np.linspace(0, 15.0, 31)

epsilon_dBms = [-10]
frequencies_to_simulate = {0: np.linspace(5.975e9, 6.085e9, 100),
                           np.pi: np.linspace(6.0153e9, 6.046e9, 40)}

for epsilon_dBm in epsilon_dBms:
  for phase in phases:
    frequencies = frequencies_to_simulate[phase]
    create_plots_for_phase(frequencies, attenuations, phase, epsilon_dBm=epsilon_dBm, gain_offset = gain_offset)
