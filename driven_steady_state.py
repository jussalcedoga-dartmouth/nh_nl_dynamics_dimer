...
def func(alpha, omega_d, phase, attenuation, epsilon_dBm = -10):
    ...
    d_alpha2 = -(((kappa_cavity2) - (G1)*np.sqrt(t_hop_1_2 * t_hop_2_2)* np.exp(-1j * phase)) + 1j * (omega2 - omega_d)) * alpha2_c + 1j * G1 * np.sqrt(t_hop_1_2 * t_hop_2_2) * alpha1_c * np.exp(-1j * phase) + epsilon
