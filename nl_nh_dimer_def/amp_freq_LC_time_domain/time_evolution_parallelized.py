...
def solve_and_plot(args):
    ...
    plot_folder = f'time_evolution_LC_no_abs/'
    os.makedirs(plot_folder, exist_ok=True)

    if save_plots:
        # Define the plot folder based on net_gain
        plot_folder = f'time_evolution_LC/undriven_time_domain_net_gain_{net_gain:.2f}'
        os.makedirs(plot_folder, exist_ok=True)
    else:
        pass
...
def main():
    ...
    # Save results to CSV
    df = pd.DataFrame(results, columns=['Gain [dB]', 'Phase [rads.]', 'Amplitude [dBm]', 'Frequency [Hz]'])
    df.to_csv('time_evolution_LC/results.csv', index=False)
