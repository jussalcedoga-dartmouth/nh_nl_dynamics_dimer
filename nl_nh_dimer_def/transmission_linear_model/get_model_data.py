...
def get_data_for_phase(frequencies, net_gains, phase, epsilon_dBm):
    ...
    for j, net_gain in enumerate(net_gains):
        for i, omega_d_val in enumerate(frequencies):
            ...
            N2_dB = 10*np.log10(((omega_d_val * kappa_drive * kappa_readout) / omega_d_val) * (N2_total / epsilon**2))
...
def get_phase_diagram(ax):
    ...
    # Ensure the pivot operation results in a DataFrame and then convert to numpy array if needed
    amplitude_matrix_reshaped = loaded_df.pivot(index='Phase [rads.]', columns='Gain [dB]', values='Amplitude [dBm]').to_numpy()
    frequency_matrix_reshaped = loaded_df.pivot(index='Phase [rads.]', columns='Gain [dB]', values='Frequency [GHz]').to_numpy()
    ...
    # Copy original matrices for comparison
    amplitude_matrix_original = np.copy(amplitude_matrix_reshaped)
    frequency_matrix_original = np.copy(frequency_matrix_reshaped)
