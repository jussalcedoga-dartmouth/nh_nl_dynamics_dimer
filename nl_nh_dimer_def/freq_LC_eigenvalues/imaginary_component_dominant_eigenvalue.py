...
def fixed_points(phase_val, gain, initial_guess):
    ...
    sol = root(func_to_optimize, initial_guess, 
               jac=lambda x: jacobian_numerical(x, phase_val, gain), tol=1e-8, method='hybr')
    if sol.success:
...
cbar_eig3 = plt.colorbar(contour_eig3)
locator = MaxNLocator(nbins=6)  # Adjust 'nbins' to change number of ticks
cbar_eig3.locator = locator
cbar_eig3.update_ticks()
cbar_eig3.ax.tick_params(labelsize=20)
cbar_eig3.ax.set_ylabel(r'$|\Delta \omega_{+}|$ [MHz]', fontsize=20)  # Increase label size
...
plt.gca().set_xlabel(r'$\Delta G$ [dB]', fontsize=35)
