...
folder_name = 'results_symm_gain_bb'
...
kappa_0_1 = 7.04e6 - 2e6
kappa_0_2 = 8.73e6 - 2e6
...
def func(alpha, omega_d, phase, attenuation, epsilon_dBm = -10):
    ...
    d_alpha2 = -(((kappa_cavity1) - (G1)*np.sqrt(t_hop_1_2 * t_hop_2_2)*np.exp(-1j * phase)) + 1j * (omega2 - omega_d)) * alpha2_c + 1j * G1 * np.sqrt(t_hop_1_2 * t_hop_2_2) * alpha1_c * np.exp(-1j * phase) + epsilon
...
alpha = [2000, 0, 2000, 0]
phases = [0, np.pi]
attenuations = np.linspace(0, 8.7, 31)
